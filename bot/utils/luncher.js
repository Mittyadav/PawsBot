...
`
  ...
  async #run_tasks(tgClients) {
    ...
    if (settings.MAX_CONCURRENT_ACCOUNT > 2) {
      console.log("\n");
      logger.warning(
        `<ye>[paws]</ye> | MAX_CONCURRENT_ACCOUNT greater than 2 may cause 403 errors while running the bot\n`
      );
    ...
    const tasks = tgClients.map(async (tgClient, index) => {
      return limit(async () => {
        ...
        try {
          ...
          logger.info(
            `<ye>[paws]</ye> | ${tgClient.session_name} | Sleeping ${sleeping} seconds before starting the bot`
          );
    ...
    logger.info(
      `<ye>[paws]</ye> | All threads completed | Sleeping ${sleep_thread} seconds before rerunning threads again`
    );
    ...
    const tasks = queries?.map(async ([query_name, query_id], index) => {
      return limit(async () => {
        ...
        try {
          ...
          logger.info(
            `<ye>[paws]</ye> | ${query_name} | Sleeping ${sleeping} seconds before starting the bot`
          );
          await sleep(sleeping);
